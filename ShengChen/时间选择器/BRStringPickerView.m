...
- (void)configDataSource:(id)dataSource defaultSelValue:(id)defaultSelValue {
    ...
    }else if ([[self.dataSourceArr firstObject] isKindOfClass:[NSDictionary class]])
    {
        self.type = BRStringPickerComponentMore;
    }
    // 4. 给选择器设置默认值
    if (self.type == BRStringPickerComponentSingle) {
        ...
        if ([[self.dataSourceArr firstObject] isKindOfClass:[NSDictionary class]])
        {
            NSMutableArray* firstArr = [[NSMutableArray alloc]init];
            for (NSInteger i = 0; i<self.dataSourceArr.count; i++)
            {
                [firstArr addObject:[self.dataSourceArr[i] objectForKey:@"TypeName"]];
            }
            for (NSInteger i = 0; i<self.dataSourceArr.count; i++) {
                NSString *selValue = nil;
                selValue = [self.dataSourceArr[i] objectForKey:@"TypeName"];
                
                NSArray* secondArray = [self.dataSourceArr[i] objectForKey:@"type_list"];
                for (int j = 0; j<secondArray.count; j++) {
                    [tempArr addObject:[secondArray[j] objectForKey:@"SkillTypeName"]];
                }
                NSInteger row = [firstArr indexOfObject:selValue];
...
- (UIView *)pickerView:(UIPickerView *)pickerView viewForRow:(NSInteger)row forComponent:(NSInteger)component reusingView:(nullable UIView *)view {
    ...
    if (self.type == BRStringPickerComponentSingle) {
        ...
        {
            label.text = [[self.dataSourceArr[component] objectForKey:@"type_list"][row] objectForKey:@"SkillTypeName"];
        }
