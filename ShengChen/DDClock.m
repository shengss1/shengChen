...
-(void)drawRect:(CGRect)rect{
    ...
    }else
    {
        ...
        {
            [self drawDDClockWithTime:self.currentTime theme:_theme];
        }else
        {
            [self drawDDClockWithTime:[NSDate new] theme:_theme];
        }
...
-(void)drawImage{
    if (_theme == 0) {
        [[UIImage imageNamed:@"hwl_szs_dp"]drawAtPoint:CGPointMake(0, 0)];
    }else
    {
        [[UIImage imageNamed:@"hwl_szs_bg2"]drawAtPoint:CGPointMake(0, 0)];
    }
}
...
- (void)drawDDClockWithTime:(NSDate*)currentTime theme:(DDClockTheme)theme{
    ...
    float secondAngle;
    if (_theme == 0) {
        arr = [self dateToAngle:currentTime];
        hourAngle = [[arr objectAtIndex:0] floatValue];
        minuteAngle = [[arr objectAtIndex:1] floatValue];
        secondAngle = [[arr objectAtIndex:2] floatValue];
    }else
    {
        arr = [self dateToAngle:currentTime];
        hourAngle = [[arr objectAtIndex:0] floatValue];
    ...
    switch (theme) {
        case DDClockThemeDefault:
            ...
            rimColor = YLSColor(0, 0, 0, 0.8);
            faceColor = [UIColor clearColor];
            markColor = [UIColor clearColor];
            secondHandColor = [UIColor redColor];
            fontColor = [UIColor clearColor];
            minuteHandColor = YLSColor(199, 206, 222, 1);
            hourHandColor = YLSColor(0, 0, 0, 1);
            break;
        ...
        case DDClockThemeDark:
            ...
            fontColor = [UIColor clearColor];
            minuteHandColor = YLSColor(199, 206, 222, 1);
            hourHandColor = YLSColor(66, 74, 89, 1);
...
//    [rimPath fill];
    ...
    if (_theme == 1)
    {
        ...
        if (![YTUserInfoManager dx_isNullOrNilWithObject:[dataUser objectForKey:@"DeadDay"]])
        {
            NSString *unitYStr = [dataUser objectForKey:@"DeadDay"];
//            [unitYStr drawInRect:CGRectMake(distanceToLeft+10,10,100,10) withFont:50];
            NSMutableParagraphStyle *paragraphStyle = [[NSParagraphStyle defaultParagraphStyle] mutableCopy];
            paragraphStyle.alignment = NSTextAlignmentCenter;

            NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:[UIFont fontWithName:@"HelveticaNeue-Bold" size:80],NSFontAttributeName,[UIColor colorWithRed:255/255.0 green:255/255.0 blue:255/255.0 alpha:0.1],NSForegroundColorAttributeName,paragraphStyle,NSParagraphStyleAttributeName, nil];
            [unitYStr drawWithRect:CGRectMake(0, 55, 200, 200) options:NSStringDrawingUsesLineFragmentOrigin attributes:dict context:nil];
        }
    ...
    UIBezierPath* hourHandPath = [UIBezierPath bezierPathWithRect: CGRectMake(-0.99, -65, 2, 70)];
    ...
    UIBezierPath* minuteHandPath = [UIBezierPath bezierPathWithRect: CGRectMake(-11.01, -0.92, 90, 2)];
    ...
    if (_theme == 0)
    {
        ...
        UIBezierPath* secondHandPath = [UIBezierPath bezierPathWithRect: CGRectMake(-15.01, -0.92, 100, 1)];
    ...
    //// 时钟中间的部分
    UIBezierPath* facePath = [UIBezierPath bezierPathWithOvalInRect: CGRectMake(97.5, 97.5, 5, 5)];
    [secondHandColor setFill];
    [facePath fill];
...
//    [centreEmptyOvalPath moveToPoint: CGPointMake(98.5, 98.5)];
//    [centreEmptyOvalPath addCurveToPoint: CGPointMake(98.5, 104.26) controlPoint1: CGPointMake(93.18, 97.97) controlPoint2: CGPointMake(93.18, 101.86)];
...
//    [centreEmptyOvalPath addCurveToPoint: CGPointMake(104.26, 98.5) controlPoint1: CGPointMake(106.65, 101.86) controlPoint2: CGPointMake(106.65, 97.97)];
//    [centreEmptyOvalPath addCurveToPoint: CGPointMake(98.5, 98.5) controlPoint1: CGPointMake(101.86, 93.18) controlPoint2: CGPointMake(97.97, 93.18)];
